Attribute VB_Name = "impCSFInvChilled"
...
Dim minutesElap As String, chillCSF As String
...
Set ws = Worksheets("csfInvoicesChilled")
...
With ws
      ...
      nRows = .Cells(Rows.Count, 4).End(xlUp).Row
       ...
       .Range("Q2:Q" & nRows).Formula = "=INDEX(agmtType,MATCH(VALUE(csfInvoicesChilled!J2),sapID,0))"
       .Calculate
...
wsCSFName = Worksheets("csfInvoicesChilled").Range("P" & i)
...
With ActiveSheet
    ...
    chillCSF = "Chilled - Category Support Fund"
    
    If Cells.Find(chillCSF) Is Nothing Then
    MsgBox "Please check " & sheetName & " for value " & chillCSF & vbNewLine & _
    "The proceeding CSF invoices will need to be checked ", vbExclamation = vbOKOnly, chillCSF & " Not Found"
    GoTo unfoundCSF:
    End If
    
     .Cells.Find(What:="Chilled - Category Support Fund", After:=ActiveCell, _
   ...
   invNum = ActiveCell.Value
    ...
    ' agmt type is onegf add two cells to the right and copy paste values
        If arrAgmtType(j, 1) = "oneGF" Or arrAgmtType(j, 1) = "chilled" Then
        col1 = col1 + 2
        col2 = col1 + 2
    ...
    "The proceeding CSF will need to be checked " & vbNewLine & _
    "Press OK to continue", vbExclamation = vbOKOnly, mthSearch & " Not Found"
    GoTo unfoundCSF:
    End If
...
If Err.Number = 9 Then
    MsgBox ("Please create a Worksheet called 'csfInvoicesChilled'" & vbCr _
